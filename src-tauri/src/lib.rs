...
pub fn run() {
    let (async_proc_input_tx, async_proc_input_rx) = mpsc::channel::<Event>(1);
    let (async_proc_output_tx, mut async_proc_output_rx) = mpsc::channel::<Event>(1);
...
fn event_handler(message: Event, app_handle: &AppHandle) {
    match message {
        Event::Files(progress) => {
            app_handle.emit("download_progress", &progress).unwrap();
        }
    }
}
